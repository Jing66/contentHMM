...
class HiddenMarkovModelTagger(TaggerI):
    ...
    def _best_path(self, unlabeled_sequence):
        T = len(unlabeled_sequence)  # T = #E
        N = len(self._states)  # N=#X
        self._create_cache()
        self._update_cache(unlabeled_sequence)
        P, O, X, S = self._cache

        V = np.zeros((T, N), np.float32)  # V[t,j] = delta_j(t)
        B = -np.ones((T, N), np.int)    # B[t,j] = max{V[t-1,j]*a_ij}

        V[0] = P + O[:, S[unlabeled_sequence[0]]] #delta_j(1) = pi_j*b_j(E_1) for all j (whole row)
        for t in range(1, T):
            for j in range(N):
                vs = V[t-1, :] + X[:, j]  
                best = np.argmax(vs)
                V[t, j] = vs[best] + O[j, S[unlabeled_sequence[t]]]  #delta_j(t) = b_j(E_t)*argmax_i{delta_i(t-1)*a_ij}
                B[t, j] = best  

        current = np.argmax(V[T-1,:]) #current = argmax_i{delta_i(t-1)*a_ij}
        sequence = [current]
        for t in range(T-1, 0, -1): # append all back pointers
            last = B[t, current]
            sequence.append(last)
            current = last

        sequence.reverse()  #reverse the seq of back pointers
    ...
    def _best_path_simple(self, unlabeled_sequence):
        ...
        for i, state in enumerate(self._states):
            V[0, i] = self._priors.logprob(state) + \
                      self._output_logprob(state, symbol)   #V[0,:] = P + O[:,S[symbol]]
            B[0, state] = None
    ...
    def random_sample(self, rng, length):
        ...
        tokens = []
        state = self._sample_probdist(self._priors, rng.random(), self._states) #sample random state from prior
        symbol = self._sample_probdist(self._outputs[state],
                                  rng.random(), self._symbols)  #sample random tag from state using emission prob
    ...
    # given a random number p, take a sample from samples according to probdist
    ...
    def entropy(self, unlabeled_sequence):
        ...
        *Z = Pr(O) = sum_S Pr(S, O)}* where the summation ranges over all state
        ...
        alpha = self._forward_probability(unlabeled_sequence) # alpha[t,s] = be in state s at time t after observing seq. 0..t (include t)
        beta = self._backward_probability(unlabeled_sequence) # beta[t,s] = be in state s at time t after observing seq. t+1...T 
        normalisation = logsumexp2(alpha[T-1]) 

        entropy = normalisation

        # starting state, t = 0
        for i, state in enumerate(self._states): # i=index of state
...
class HiddenMarkovModelTrainer(object):
    ...
    # calculate one step of a_ij, b_i(E_k) by summing up responsibilities using MLE rules
    def _baum_welch_step(self, sequence, model, symbol_to_number):
        ...
        alpha = model._forward_probability(sequence)  # alpha[t,i] = P(E_1:t, X_t=i)
        ...
        prior_logp = alpha[0]+beta[0] - A_denom

        return lpk, A_numer, A_denom, B_numer, B_denom, prior_logp
    ...
    def train_unsupervised(self, unlabeled_sequences, update_outputs=True,
        ...
        while not converged and iteration < max_iterations:
            ...
            for sequence in unlabeled_sequences:
                ...
                seq_B_numer, seq_B_denom, prior_logp) = self._baum_welch_step(sequence, model, symbol_numbers)

                # add these sums to the global A and B values
                # MUST update row by row?
                # A_numer = np.logaddexp2(A_numer,seq_A_numer-lpk)
                # B_numer = np.logaddexp2(B_numer,seq_B_numer-lpk)
            ...
            for i in range(N):
                ...
                # CANNOT directly update whole row cuz _transitions is DictConditProb of MutableProb -- have to update one by one
    ...
    def train_supervised(self, labelled_sequences, estimator=None):
        ...
        N = len(self._states)
        pi = estimator(starting, N) #estimator is a function: lambda fdist, bins: MLEProbDist(fdist)    
        A = ConditionalProbDist(transitions, estimator, N) # constructed from a ConditionalFreqDist and a ProbDist factory
